...
export default function SurgonDetails() {
  const { name } = useParams();
  const [surgon, setSurgon] = useState(null);
  const [error, setError] = useState('');

  useEffect(() => {
    const fetchSurgon = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/getsurgon/${name}`);
        setSurgon(response.data);
      } catch (error) {
        console.error('Error fetching surgon details:', error);
        setError('Error fetching surgon details');
      }
    };
  ...
  return (
    <div>
      ...
      <h1 className="text-black font-medium py-7 ml-4">Surgon</h1>
      <div className='flex flex-row gap-10'>
        <div className="shadow-lg border p-4 rounded-lg">
          <img src={surgon.image} alt={surgon.name} className="surgon-image size-56 rounded-2xl" />
        </div>
        <div className="shadow-lg border p-4 rounded-lg">
          <h2 className='text-black text-2xl font-semibold'>{surgon.name}</h2>
          <p className='text-blue-500 border w-24 border-gray-400 rounded-full bg-slate-200 mb-2'>{surgon.description}</p>
          <p className="mb-2">{surgon.experience}</p>
          <p className="mb-2">{surgon.address}</p>
