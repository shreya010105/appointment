...
export default function CardiologistDetails() {
  const { name } = useParams();

  const [cardiologist, setCardiologist] = useState(null);
  const [error, setError] = useState('');

  useEffect(() => {
    const fetchCardiologist = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/getcardiologist/${name}`);
        setCardiologist(response.data);
      } catch (error) {
        console.error('Error fetching cardiologist details:', error);
        setError('Error fetching cardiologist details');
      }
    };
  ...
  return (
    ...
    <div className='flex flex-row gap-10'>
      <div className="shadow-lg border p-4 rounded-lg">
        <img src={cardiologist.image} alt={cardiologist.name} className="dentist-image size-56 rounded-2xl" />
      </div>
      <div className="shadow-lg border p-4 rounded-lg">
        <h2 className='text-black text-2xl font-semibold'>{cardiologist.name}</h2>
        <p className='text-blue-500 border w-24 border-gray-400 rounded-full bg-slate-200 mb-2'>{cardiologist.description}</p>
        <p className="mb-2">{cardiologist.experience}</p>
        <p className="mb-2">{cardiologist.address}</p>
        <div className='flex flex-col gap-5'>
          ...
          <div>
          <Link to={`/Patreg/${cardiologist.name}`}>
                <button
                
                 className="cursor-pointer rounded-full border border-blue-800 bg-blue-500 px-3 mt-2 text-white hover:bg-blue-600">
                  Appointment Booking
                </button>
              </Link>
          </div>
