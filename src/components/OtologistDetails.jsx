...
export default function OtologistDetails() {
  const { name } = useParams();
  const [otologist, setOtologist] = useState(null);
  const [error, setError] = useState('');

  useEffect(() => {
    const fetchOtologist = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/getotologist/${name}`);
        setOtologist(response.data);
      } catch (error) {
        console.error('Error fetching otologist details:', error);
        setError('Error fetching otologist details');
      }
    };
  ...
  return (
    <div>
      ...
      <h1 className="text-black font-medium py-7 ml-4">Otologist</h1>
      <div className='flex flex-row gap-10'>
        <div className="shadow-lg border p-4 rounded-lg">
          <img src={otologist.image} alt={otologist.name} className="otologist-image size-56 rounded-2xl" />
        </div>
        <div className="shadow-lg border p-4 rounded-lg">
          <h2 className='text-black text-2xl font-semibold'>{otologist.name}</h2>
          <p className='text-blue-500 border w-24 border-gray-400 rounded-full bg-slate-200 mb-2'>{otologist.description}</p>
          <p className="mb-2">{otologist.experience}</p>
          <p className="mb-2">{otologist.address}</p>
