...
export default function GeneralDetails() {
  ...
  useEffect(() => {
    const fetchGeneral = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/getgeneral/${name}`);
        setGeneral(response.data);
      } catch (error) {
        console.error('Error fetching general details:', error);
        setError('Error fetching general details');
      }
    };
  ...
  return (
    <div>
      ...
      <h1 className="text-black font-medium py-7 ml-4">General</h1>
      <div className='flex flex-row gap-10'>
        <div className="shadow-lg border p-4 rounded-lg">
          <img src={general.image} alt={general.name} className="general-image size-56 rounded-2xl" />
        </div>
        <div className="shadow-lg border p-4 rounded-lg">
          <h2 className='text-black text-2xl font-semibold'>{general.name}</h2>
          <p className='text-blue-500 border w-24 border-gray-400 rounded-full bg-slate-200 mb-2'>{general.description}</p>
          <p className="mb-2">{general.experience}</p>
          <p className="mb-2">{general.address}</p>
