...
export default function EyeDetails() {
  ...
  useEffect(() => {
    const fetchEye = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/geteye/${name}`);
        setEye(response.data);
      } catch (error) {
        console.error('Error fetching eye details:', error);
        setError('Error fetching eye details');
      }
    };
    ...
    <div>
      ...
      <div className='flex flex-row gap-10'>
        <div className="shadow-lg border p-4 rounded-lg">
          <img src={eye.image} alt={eye.name} className="eye-image size-56 rounded-2xl" />
        </div>
        <div className="shadow-lg border p-4 rounded-lg">
          <h2 className='text-black text-2xl font-semibold'>{eye.name}</h2>
          <p className='text-blue-500 border w-24 border-gray-400 rounded-full bg-slate-200 mb-2'>{eye.description}</p>
          <p className="mb-2">{eye.experience}</p>
          <p className="mb-2">{eye.address}</p>
