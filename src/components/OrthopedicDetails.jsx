...
export default function OrthopedicDetails() {
  const { name } = useParams();
  const [orthopedic, setOrthopedic] = useState(null);
  const [error, setError] = useState('');

  useEffect(() => {
    const fetchOrthopedic = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/getorthopedic/${name}`);
        setOrthopedic(response.data);
      } catch (error) {
        console.error('Error fetching orthopedic details:', error);
        setError('Error fetching orthopedic details');
      }
    };
  ...
  return (
    <div>
      ...
      <h1 className="text-black font-medium py-7 ml-4">Orthopedic</h1>
      <div className='flex flex-row gap-10'>
        <div className="shadow-lg border p-4 rounded-lg">
          <img src={orthopedic.image} alt={orthopedic.name} className="orthopedic-image size-56 rounded-2xl" />
        </div>
        <div className="shadow-lg border p-4 rounded-lg">
          <h2 className='text-black text-2xl font-semibold'>{orthopedic.name}</h2>
          <p className='text-blue-500 border w-24 border-gray-400 rounded-full bg-slate-200 mb-2'>{orthopedic.description}</p>
          <p className="mb-2">{orthopedic.experience}</p>
          <p className="mb-2">{orthopedic.address}</p>
          <div className='flex flex-col gap-5'>
            ...
            <div>
            <Link to={`/Patreg/${orthopedic.name}`}>
